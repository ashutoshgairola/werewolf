...
export function GameRules() {
  ...
  return (
    <div className="bg-parchment-light border border-wood/40 rounded-xl overflow-hidden">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-wood/5 transition-colors"
      >
        <span className="font-tavern text-wood-dark text-sm uppercase tracking-widest">📜 How to Play</span>
        <span className="text-wood/50 text-sm">{open ? '▲' : '▼'}</span>
      </button>

      {open && (
        <div className="px-4 pb-4 space-y-4 border-t border-wood/20 pt-3">
          {/* Goal */}
          <div className="text-center bg-parchment border border-wood/20 rounded-lg px-3 py-2">
            <p className="font-tavern text-wood-dark text-sm">The Goal</p>
            <p className="text-wood text-xs font-body mt-0.5">
          ...
          {/* Roles */}
          <div>
            <p className="text-xs font-body text-wood/60 uppercase tracking-widest mb-2">Roles</p>
            <div className="space-y-2">
              {ROLES.map((r) => (
                <div key={r.name} className="flex gap-2.5 items-start">
                  <span className="text-lg flex-shrink-0 mt-0.5">{r.icon}</span>
                  <div>
                    <span className="font-body text-ink text-sm font-semibold">{r.name} — </span>
                    <span className="font-body text-wood text-sm">{r.desc}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Phases */}
          <div>
            <p className="text-xs font-body text-wood/60 uppercase tracking-widest mb-2">Each Round</p>
            <div className="space-y-2">
              {PHASES.map((p) => (
                <div key={p.name} className="flex gap-2.5 items-start">
                  <span className="text-lg flex-shrink-0 mt-0.5">{p.icon}</span>
                  <div>
                    <span className="font-body text-ink text-sm font-semibold">{p.name} — </span>
                    <span className="font-body text-wood text-sm">{p.desc}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Tips */}
          <div className="bg-candle/10 border border-candle/30 rounded-lg px-3 py-2 text-xs font-body text-wood space-y-1">
            <p className="font-semibold text-wood-dark">💡 Tips</p>
