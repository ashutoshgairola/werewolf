...
export function ChatPanel({ defaultChannel = 'day', visibleChannels }: ChatPanelProps) {
  ...
  return (
    <div className="relative flex flex-col h-full bg-parchment border border-wood/30 rounded-xl overflow-hidden">
      ...
      <div className="flex border-b border-wood/20 bg-parchment-light overflow-x-auto">
        {tabs.map((ch) => (
          <button
            ...
            className={[
              'px-3 py-2 text-xs font-body whitespace-nowrap flex-shrink-0 border-b-2 transition-colors',
              activeChannel === ch
                ? 'border-candle text-wood-dark font-semibold'
                : 'border-transparent text-wood/50 hover:text-wood',
            ].join(' ')}
          >
            {CHANNEL_LABELS[ch]}
          </button>
      ...
      >
        {messages.length === 0 && (
          <p className="text-center text-wood/40 text-xs font-body py-8 italic">No messages yet</p>
        )}
        {messages.map((msg) => (
      ...
      {newBadge && !atBottom && (
        <button
          ...
          className="absolute bottom-16 left-1/2 -translate-x-1/2 bg-candle text-ink text-xs rounded-full px-3 py-1 shadow"
        >
          ↓ New message
        </button>
      )}
      ...
      {/* Rate limit toast */}
      {rateLimitMsg && (
        <div className="px-3 py-1.5 bg-ember/10 border-t border-ember/30 text-xs text-ember font-body text-center">
          ⚠ {rateLimitMsg}
        </div>
      )}
      ...
      {/* Input */}
      {canSend && (
        <div className="border-t border-wood/20 p-2 flex gap-2">
          <textarea
            ref={inputRef}
            value={text}
            ...
            rows={1}
            className="flex-1 resize-none bg-parchment border border-wood/30 rounded-lg px-3 py-2 text-sm font-body text-ink focus:outline-none focus:ring-1 focus:ring-candle"
            style={{ maxHeight: '80px', overflowY: 'auto' }}
          />
          <button
            onClick={send}
            disabled={!text.trim()}
            className="px-3 py-2 bg-candle rounded-lg text-sm font-body text-ink disabled:opacity-40"
          >
            ↵
          </button>
        </div>
